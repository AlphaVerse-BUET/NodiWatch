...
| **Python FastAPI Backend** | ~100-200 MB RAM | **FREE** (Render/Railway free tier) | Just sends requests to GEE, returns results |
...
- Replace the "7 Microservice" diagram with a **lean 3-tier architecture**:
  ...
  2. **FastAPI (thin API)** → one small Python server, ~200 lines of code
...
These JavaScript scripts run directly in the GEE Code Editor during the finals. **No backend needed.**
...
### Component 3: Python Backend (NICE-TO-HAVE)

#### [NEW] `backend/main.py`
Thin FastAPI app (~200 lines):
- `/api/factories` → queries Overpass API, returns real OSM data
- `/api/spectral-indices` → calls GEE Python API, returns real index values
- `/api/water-mask` → returns GEE tile URL for MNDWI overlay

Memory: ~100MB total (FastAPI + earthengine-api). Deployable on Render free tier.
